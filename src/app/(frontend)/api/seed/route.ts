...
export async function POST() {
  try {
    console.log('[Horizon Seeder API] Initializing Payload CMS Seeding process...')
    const payload = await getPayload({ config: configPromise })

    const seededRecords: string[] = []
  ...
  <circle cx="960" cy="540" r="600" fill="#DC2626" opacity="0.1" filter="blur(100px)"/>
    ...
    if (!mediaRecord) {
      ...
      seededRecords.push('Media: Horizon Enterprise Hero')
    }
    ...
    if (existingSlides.totalDocs === 0) {
      ...
      seededRecords.push('Hero Slide: Transforming Enterprise Infrastructure')
    }
    ...
    for (const s of servicesToSeed) {
      ...
      if (existing.totalDocs === 0) {
        ...
        seededRecords.push(`Service: ${s.title}`)
      }
    }
    ...
    for (const c of clientsToSeed) {
      ...
      if (existing.totalDocs === 0) {
        ...
        seededRecords.push(`Client: ${c.name}`)
      }
    }

    console.log(`[Horizon Seeder API] Successfully seeded ${seededRecords.length} records.`)

    return NextResponse.json({
      success: true,
      message: `Database seeding completed successfully!`,
      seededCount: seededRecords.length,
      records: seededRecords,
    })
  } catch (error: any) {
    console.error('[Horizon Seeder API Error]', error)
